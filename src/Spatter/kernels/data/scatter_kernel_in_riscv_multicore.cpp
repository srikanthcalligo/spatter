...
void kernel_main()
{
    ...
    //DPRINT << "ntw = " << num_tiles_written << " notpc= " << num_output_tiles_per_core << ENDL();
    ...
    uint32_t* dat2 = (uint32_t*) l1_write_addr_in0;
    uint32_t outer_loop_count = 1024 / pattern_length; // 32 * 32
    //DPRINT << "ublock = " << ublock_size_bytes_2 << ENDL();
    //dense[j + pattern_length * (i % wrap)] = sparse[pattern[j] + delta * i];
    //for(uint32_t tile_id = 0; tile_id < (num_output_tiles_per_core); tile_id++) {
    for(uint32_t tile_id = core_id*num_output_tiles_per_core; tile_id < (core_id*num_output_tiles_per_core+num_output_tiles_per_core); tile_id++) {
        ...
        //noc_async_read(src0_dram_noc_addr, l1_write_addr_in0, ublock_size_bytes_0);
        noc_async_read_barrier();
        uint32_t* dat0 = (uint32_t*) l1_write_addr_out0;
