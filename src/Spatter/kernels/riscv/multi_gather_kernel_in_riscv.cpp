...
void kernel_main()
{
    ...
    constexpr uint32_t dense_cb_id1 = tt::CBIndex::c_3;

    uint32_t sparse_tile_size = get_tile_size(sparse_cb_id0);
    uint32_t pattern_tile_size = get_tile_size(pattern_cb_id1);
    uint32_t pattern_gather_tile_size = get_tile_size(pattern_cb_id1);
    ...
    uint32_t* data_pattern_gather = (uint32_t*) pattern_gather_l1_write_addr_in1;
    ...
    for(uint32_t tile_id = 0; tile_id < n_tiles; tile_id++) {
        ...
        for(uint32_t i = 0; i < loop_count; i++){
            for(uint32_t j = 0; j < pattern_length; j++){
                *(compute_dense + (j + pattern_length * (i % wrap))) = *(data + (*( data_pattern +(*(data_pattern_gather+j))) + delta * i));
            }
        }
