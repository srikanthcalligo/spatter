...
void kernel_main()
{
    ...
    uint32_t dram_addr_dense = get_arg_val<uint32_t>(0);
    uint32_t dram_addr_pattern = get_arg_val<uint32_t>(1);
    uint32_t n_tiles =  get_arg_val<uint32_t>(2);
    uint32_t dram_addr_sparse = get_arg_val<uint32_t>(3);
    ...
    noc_async_read(dram_addr_dense, l1_write_addr_in0, ublock_size_bytes_0);
    noc_async_read_barrier();
    noc_async_read(dram_addr_pattern, l1_write_addr_in1, ublock_size_bytes_1);
    noc_async_read_barrier();

    uint32_t* data_pattern = (uint32_t*) l1_write_addr_in1;
    uint32_t* data_dense = (uint32_t*) l1_write_addr_in0;
    uint32_t outer_loop_count = single_tile_size / delta;
    if(extra_tile){
        outer_loop_count = outer_loop_count - stride;
    }
    //Iterate through all tiles and fill sparse array with pattern data
    for(uint32_t tile_id = 0; tile_id < n_tiles; tile_id++) {
        ...
        noc_async_write_tile(tile_id, src_a_buf, l1_write_addr_out0);
