...
void kernel_main()
{
    ...
    constexpr uint32_t sparse_cb_id0 = tt::CBIndex::c_2;
    constexpr uint32_t pattern_cb_id1 = tt::CBIndex::c_1;
    constexpr uint32_t dense_cb_id1 = tt::CBIndex::c_0;
    ...
    noc_async_read(dense_dram_addr, dense_l1_write_addr_in1, dense_tile_size);
    noc_async_read_barrier();
    ...
    for(uint32_t tile_id = num_tiles_written; tile_id < (num_tiles_written+num_output_tiles_per_core); tile_id++) {
        ...
        //noc_async_read_tile(tile_id, sparse_src_buf, cb_in0_addr); // read the tile into the L1
        //noc_async_read_barrier();
        ...
        for(uint32_t i = 0; i < loop_count; i++){
            for(uint32_t j = 0; j < pattern_length; j++){
                *(data + (*(data_pattern+j) + delta * i)) = *(compute_dense + (j + pattern_length * (i % wrap)));
            }
        }
