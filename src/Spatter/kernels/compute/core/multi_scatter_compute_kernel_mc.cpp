...
void MAIN {
    ...
    uint32_t num_tiles_written = get_arg_val<uint32_t>(8);
    uint32_t num_output_tiles_per_core = get_arg_val<uint32_t>(9);
    ...
    for(uint32_t tile_id = num_tiles_written; tile_id < (num_tiles_written+num_output_tiles_per_core); tile_id++)
    {
        ...
        for(uint32_t i = 0; i < loop_count; i++){
            ...
            for(uint32_t j = 0; j < pattern_length; j++){
                sparse_addr_ptr[4 + (pattern_addr_ptr[4 + scatter_pattern_addr_ptr[4 + j]] + (delta * i))] = dense_addr_ptr[4 + (j + pattern_length * (i % wrap))]; //Need to add 4 because read ptr is off by 1 << 4
            }
        }
