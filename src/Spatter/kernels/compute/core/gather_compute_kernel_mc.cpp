...
void MAIN {
    ...
    for(uint32_t tile_id = num_tiles_written; tile_id < (num_tiles_written+num_output_tiles_per_core); tile_id++) {
        ...
        for(uint32_t i = 0; i < loop_count; i = i + 1){
            sparse_index = i * pattern_length;
            #pragma GCC unroll 8
            for(uint32_t j = 0; j < pattern_length; j++){
                //pattern_addr_ptr contains all index values (pattern_addr_ptr[4+j] + delta * i))
                dense_addr_ptr[j] = sparse_addr_ptr[pattern_addr_ptr[ sparse_index + j]];
                //dense_addr_ptr[(j + pattern_length * (i % wrap))] = sparse_addr_ptr[(pattern_addr_ptr[j] + delta * i)]; //Need to add 4 because read ptr is off by 1 << 4
            }
        }
