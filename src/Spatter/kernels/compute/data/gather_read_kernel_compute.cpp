...
void kernel_main()
{
    ...
    uint32_t compute_pattern_l1_write_addr_in1 = get_write_ptr(compute_pattern_cb_id1);
    ...
    uint32_t* compute_sparse = (uint32_t*) compute_pattern_l1_write_addr_in1;
    ...
    for(uint32_t tile_id = 0; tile_id < n_tiles; tile_id++) {
        ...
        for(uint32_t i = 0; i < loop_count; i++){
            for(uint32_t j = 0; j < pattern_length; j++){
                ...
                *(compute_sparse + (dense_index % single_tile_size)) = *(data + index_1);
                
                //DPRINT << dense_index << " " << index_1 << " " << *(compute_sparse + (dense_index % single_tile_size)) << ENDL();
